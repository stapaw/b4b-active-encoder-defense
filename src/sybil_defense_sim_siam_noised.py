...
def train(epoch, model, train_loader, criterion, optimizer, device, mapper=False):
    model.train()
    for batch_idx, (data, target) in enumerate(train_loader):
        optimizer.zero_grad()
        output = model(data.to(device).float())
        if len(target.shape) > 1 and not mapper:
...
def run(cfg: Config):
    ...
    embeddings_dataset_train2: EncodingsToLabels = torch.load(cfg.benchmark.embeddings.train_path2)
    ...
    embeddings_dataset_train2.encodings = embeddings_dataset_train2.encodings * 1000
    ...
    transformedB_embeddings_train = get_transformed_embeddings(embeddings_dataset_train2.encodings, transformB)
    transformedB_embeddings_test = get_transformed_embeddings(embeddings_dataset_test.encodings, transformB)

    transformedB_dataset_train = EncodingsToLabels(transformedB_embeddings_train, embeddings_dataset_train2.labels)
    ...
    for n_items in cfg.benchmark.subsets:
        ...
        batch_size = 64
        mapper_epochs = cfg.benchmark.hparams.train_epochs_mapper
        ...
        for epoch in range(1, mapper_epochs):
            train(epoch, mapper, train_loader, criterion, optimizer, device=device, mapper=True)
