...
@hydra.main(
    config_path="../config", config_name="config_reference_model.yaml", version_base="1.2"
)
...
def run(cfg: Config):
    ...
    acc_results= []
    for _ in range(RUNS):
        train_loader = DataLoader(embeddings_dataset_train, batch_size=64, shuffle=True)
        test_loader = DataLoader(embeddings_dataset_test, batch_size=64, shuffle=False)

        reference_model = Net(embeddings_dataset_train.encodings[0].shape[0], cfg.benchmark.embeddings.class_number).to(device)
