...
def train(epoch, model, train_loader, criterion, optimizer, device):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss = criterion(output, target.type(torch.LongTensor).to(device))
        loss.backward()
        optimizer.step()
...
def run(cfg: Config):
    ...
    for _ in range(RUNS):
        ...
        print(torch.mean(embeddings_dataset_train.encodings), torch.std(embeddings_dataset_train.encodings, dim=1),
              torch.sum(embeddings_dataset_train.encodings))
        ...
        reference_model = Net(transformedA_dataset_train[0][0].shape[0], cfg.benchmark.embeddings.class_number).to(
            device)
